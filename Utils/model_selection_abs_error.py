...
def analyze_predictions_list_database(model, dir_name,model_name,i=None):
    ...
    for file_name in os.listdir(dir_name):
        if os.path.isfile(dir_name+file_name) == True:
            ...
            if i != None:
                all_predictions_df.to_csv(model_name+'_'+i+'.csv')
            else:
                all_predictions_df.to_csv(model_name+'.csv')
...
df_out.to_csv('model_parameters_abss_error.csv')
...
parameter_sumamry = pd.read_csv('Absolute error/model_parameters_abss_error.csv')
